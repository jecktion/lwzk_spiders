...
class Spider(object):
	...
	def get_t_movie(self, film_url, product_number, plat_number,table_name):
		film_url = self.normal_url(film_url)
		retry = 5
		while 1:
			try:
				...
				try:
					...
					area = ''
				try:
					p2 = re.compile(u'<li class="p-row p-renew">(\d+?)集全 </li>', re.S)
					sets_number = re.findall(p2, text)[0]
				except:
					try:
						p2 = re.compile(u'更新至(\d+?)集')
						sets_number = re.findall(p2, text)[0]
					except:
						sets_number = '1'
				try:
					p3 = re.compile(u'<li>类型：(.*?)</li>')
					product_type = self.replace(re.findall(p3, text)[0])
				except:
					product_type = ''
				try:
					p = re.compile(u'<li class="p-performer" title="(.*?)">')
					actors = re.findall(p, text)[0].replace('/', ';')
				except:
					actors = ''
				try:
					p4 = re.compile(u'<span class="intro-more hide">简介：(.*?)</span>', re.S)
				...
				try:
					...
					try:
						p5 = re.compile(u'<span class="sub-title">(.*?)</span>', re.S)
						first_show_year_global = re.findall(p5, text)[0]
					except:
						first_show_year_global = ''
				try:
					product_image = self.get_md5(product_number + plat_number)
					p6 = re.compile(u'<i class="bg"></i><img src="(.*?)"')
		...
		while 1:
			try:
				...
				if '<meta name="description" content="' not in text:
					retry -= 1
					if retry == 0:
						print 'error|' + film_url
						return None
					else:
						continue
				try:
					p1 = re.compile(u'<span class="star-num">(.*?)</span>')
					score = re.findall(p1, text)[0]
				except:
					score = '0'
				try:
					p2 = re.compile(u'<li>评论：([0-9,]+?)</li>', re.S)
					comment_num = re.findall(p2, text)[0].replace(',', '')
				except:
					comment_num = '0'
				try:
					p2 = re.compile(u'<li>总播放数：([0-9,]+?)</li>', re.S)
