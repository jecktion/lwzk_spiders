...
class Spider(object):
	...
	def get_headers(self):
		...
		headers = {'Host': 'www.shilladfs.com',
	...
	def get_t_move_summary(self, film_url, product_number, plat_number,table_name):
		retry = 5
		while 1:
			try:
				...
				play_num = '0'
				try:
					p2 = re.compile(u'<span class="m-span">累计票房<br />(.*?)</span>', re.S)
					ticket_num = re.findall(p2, text)[0].replace(',', '')
					if u'万' in ticket_num:
						ticket_num = float(ticket_num.replace(u'万', '')) * 10000
						ticket_num = '%.0f' % ticket_num
					if u'亿' in ticket_num:
						ticket_num = float(ticket_num.replace(u'亿', '')) * 100000000
						ticket_num = '%.0f' % ticket_num
				except:
					ticket_num = '0'
...
if __name__ == "__main__":
	spider = Spider()
	# spider.get_t_move_summary(' http://www.cbooo.cn/m/616870', 'B0000492', 'P37', 'T_MOVIE_SUMMARY')
	s = []
	...
	for j in s:
		print j[0],j[1]
		try:
			spider.get_t_move_summary(j[1], j[0], 'P37','T_MOVIE_SUMMARY')
		except:
			continue
	spider.db.db.close()
